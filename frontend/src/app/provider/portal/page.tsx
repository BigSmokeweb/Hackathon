...
export default function ProviderPortalPage() {
  ...
  return (
    <div className="bg-[#F7F4EA] text-[#3E4541] min-h-screen pt-28 pb-24 selection:bg-[#4FA3D1]/30 selection:text-[#3E4541]">
      {/* Toast notification */}
      {toast && (
        <div className="fixed top-20 left-1/2 -translate-x-1/2 z-50 px-5 py-3 bg-[#347F8C] text-[#F7F4EA] text-xs font-mono rounded-xl shadow-lg shadow-[#347F8C]/30 flex items-center gap-2">
          <CheckCircle2 className="w-4 h-4 text-[#8FAF82]" />
          {toast}
        </div>
      )}
          ...
          <div className="space-y-8">
            <div className="bg-white p-6 sm:p-8 rounded-2xl border border-[#D8D4C8] shadow-sm flex flex-col sm:flex-row sm:items-center justify-between gap-6">
              <div>
                ...
                <h2 className="font-manifold text-xl sm:text-2xl text-[#3E4541] font-bold uppercase tracking-wide">Curated Experience Portfolio</h2>
              ...
              <button
                ...
                className="inline-flex items-center justify-center gap-2 bg-[#347F8C] hover:bg-[#2A6772] text-[#F7F4EA] text-xs font-mono uppercase tracking-wider font-bold px-5 py-3 rounded-xl shadow-md shadow-[#347F8C]/20 transition-all active:scale-95 shrink-0"
              >
            ...
            <div className="bg-white rounded-2xl border border-[#D8D4C8] overflow-hidden shadow-sm">
              <div className="p-5 sm:p-6 border-b border-[#D8D4C8] bg-[#F7F4EA]/40 flex items-center justify-between">
                <div>
                  ...
                  <p className="text-xs font-mono text-[#3E4541]/60 mt-0.5">Synchronized across Ahmedabad, Mumbai, Jaipur</p>
                </div>
              ...
              <div className="divide-y divide-[#D8D4C8]">
                {MOCK_LISTINGS.map((item) => (
                  <div key={item.id} className="p-5 sm:p-6 space-y-3 hover:bg-[#F7F4EA]/30 transition-colors">
                    <div className="flex flex-col md:flex-row md:items-center justify-between gap-5">
                      <div className="flex items-start gap-4 min-w-0">
                        ...
                        <div className="min-w-0">
                          <div className="flex items-center gap-2 mb-1">
                            ...
                            <span className="text-[10px] font-mono text-[#3E4541]/60 uppercase">{item.city}</span>
                          </div>
                          <h4 className="font-manifold text-base sm:text-lg text-[#3E4541] font-bold uppercase tracking-wide truncate">{item.title}</h4>
                      ...
                      <div className="flex items-center justify-between md:justify-end gap-4 shrink-0 pt-3 md:pt-0 border-t md:border-t-0 border-[#D8D4C8]">
                        <span className={`inline-flex items-center gap-1 px-3 py-1 border text-xs font-mono uppercase tracking-wider font-bold rounded-full ${
                          item.published ? 'bg-[#8FAF82]/20 text-[#347F8C] border-[#8FAF82]/30' : 'bg-amber-50 text-amber-700 border-amber-200'
                        }`}>
                          <span className={`w-1.5 h-1.5 rounded-full ${item.published ? 'bg-[#8FAF82]' : 'bg-amber-400'}`} />
                    ...
                    {item.nudges.length > 0 && (
                      <div>
                        ...
                        >
                          ...
                          <span className="text-[#D8D4C8] ml-1">{expandedNudges[item.id] ? '▲' : '▼'}</span>
                        </button>
        ...
        {activeTab === 'create' && (
          <div className="space-y-6">
            <div>
              ...
              <h2 className="font-manifold text-2xl sm:text-3xl text-[#3E4541] font-bold uppercase tracking-wide">Create a New Experience</h2>
          ...
          <div className="grid grid-cols-1 lg:grid-cols-12 gap-10">
            <div className="lg:col-span-8 bg-white p-6 sm:p-8 rounded-2xl border border-[#D8D4C8] shadow-sm">
              <div className="mb-6">
                ...
                <h2 className="font-manifold text-2xl text-[#3E4541] uppercase tracking-wide font-bold">Host & Guide Verification Audit</h2>
              ...
              {uploadSuccess ? (
                <div className="p-6 rounded-xl bg-[#8FAF82]/15 border border-[#8FAF82]/40 text-xs font-mono text-[#3E4541] space-y-3">
                  ...
                  <p className="text-[#3E4541]/80">Your verification record has been timestamped and encrypted into the host registry.</p>
                ...
                <form onSubmit={handleUploadSubmit} className="space-y-6">
                  <div>
                    <label className="block text-xs font-mono uppercase tracking-wider text-[#3E4541] font-bold mb-2">Accreditation Document Type</label>
                  ...
                  <div className="border-2 border-dashed border-[#D8D4C8] rounded-2xl p-8 sm:p-10 text-center bg-[#F7F4EA]/40 hover:bg-[#F7F4EA]/80 transition-colors">
                    ...
                    <p className="text-xs sm:text-sm font-mono uppercase tracking-wider font-bold text-[#3E4541]">Select PDF or High-Resolution Scanned License</p>
                    <p className="text-[11px] font-mono text-[#3E4541]/60 mt-1">Max file size: 15MB. Encrypted in transit via AES-256 GCM.</p>
                    <input type="file" required className="mt-4 text-xs font-mono text-[#3E4541]/80 file:mr-4 file:py-2 file:px-4 file:rounded-lg file:border file:border-[#D8D4C8] file:text-xs file:font-mono file:uppercase file:bg-white file:text-[#347F8C] file:font-bold hover:file:bg-[#F7F4EA] file:cursor-pointer cursor-pointer" />
                  </div>
            ...
            <div className="lg:col-span-4 space-y-6">
              <div className="bg-white p-6 rounded-2xl border border-[#D8D4C8] shadow-sm">
                ...
                <h3 className="font-manifold text-lg text-[#3E4541] font-bold uppercase tracking-wide">Ethical Host Charter</h3>
        ...
        {activeTab === 'mfa' && (
          <div className="bg-white p-6 sm:p-8 rounded-2xl border border-[#D8D4C8] shadow-sm max-w-2xl">
            <div className="mb-6">
              ...
              <h2 className="font-manifold text-2xl text-[#3E4541] uppercase tracking-wide font-bold">Two-Factor Authentication (MFA)</h2>
              <p className="text-xs font-mono text-[#3E4541]/70 mt-1 leading-relaxed">
            ...
            <div className="bg-[#8FAF82]/15 border border-[#8FAF82]/40 p-5 rounded-xl flex items-start gap-3.5 mb-6 text-xs font-mono text-[#3E4541]">
              ...
              <div>
                ...
                <p className="text-[#3E4541]/80 mt-1">Time-based one-time password verification (Google Authenticator / 1Password) is permanently bound to this host profile.</p>
              </div>
            </div>
            <div className="p-5 rounded-xl border border-[#D8D4C8] bg-[#F7F4EA]/40 space-y-4">
              <div className="flex items-center justify-between">
                <div>
                  <h4 className="text-xs font-mono uppercase tracking-wider font-bold text-[#3E4541]">Hardware Token / Backup Codes</h4>
                  <p className="text-[11px] font-mono text-[#3E4541]/60 mt-0.5">10 single-use emergency recovery keys provisioned</p>
                </div>
                <button type="button" className="px-3.5 py-1.5 text-xs font-mono uppercase tracking-wider bg-white border border-[#D8D4C8] text-[#347F8C] font-bold rounded-lg hover:bg-[#F7F4EA] transition shadow-sm">Regenerate</button>
              </div>
              <div className="pt-3 border-t border-[#D8D4C8] flex items-center justify-between">
                <div>
                  <h4 className="text-xs font-mono uppercase tracking-wider font-bold text-[#3E4541]">Session Sign-Out</h4>
                  <p className="text-[11px] font-mono text-[#3E4541]/60 mt-0.5">Revoke all active browser sessions across devices</p>
                </div>
